...
| **Five Beats** | Software's life is five typed beats: Declare, Prove, Render, Converge, **Attest continuously**. Sharpens the earlier four-beat formulation by making explicit the continuously-renewed runtime attestation. All five are constructive, checkpointed, typed. | THEORY.md §I.1; CONTINUOUS-SOLUTION-MACHINE.md §I.2 |
| **Continuous Solution Machine** | The runtime view of pleme-io: Kubernetes (engenho-principal) as the universal fixed-point operator for typed business state. The cluster IS the business algebra; controllers are the operators; the reconciliation log IS the business audit trail. Every reducible business concern becomes a typed CRD + Rust controller. The named methodology IS the Viggy Method (below). | THEORY.md §IV.7; CONTINUOUS-SOLUTION-MACHINE.md |
| **The Viggy Method** (a.k.a. **Viggy**) | The canonical name for pleme-io's default development mode at runtime. Etymology: from Portuguese *vigência*, "in force / currently valid" — the legal/contractual sense of a promise currently and continuously enforced. The fleet-side methodology peer of Blackmatter Development (SDLC) and Constructive Substrate Engineering (typescape). Operationalized by the ★★ CONTINUOUS CONVERGENCE + ★★ PROVABLE OUTCOMES directives. Adjective: Viggy-native. Verbal: "operating under Viggy." | THEORY.md §IV.7; CONTINUOUS-SOLUTION-MACHINE.md |
| **Fractal controller pattern** | The Seven-Beat Convergence Tick shape recurs across engenho, magma, pangea-operator, FluxCD, caixa, saguão, cofre, mado, blackmatter, tatara-reconciler, LLM-as-agent, and promessa. Kubernetes is the principal instance; everything else is a peer reconciler at a different layer with the same shape. | THEORY.md §VIII.9; CONTINUOUS-SOLUTION-MACHINE.md §II.5, §XI |
| **Seven-Beat Convergence Tick** | The runtime micro-loop every PromessaController executes: Observe → Diff → Classify → Decide → Act → Attest → Tick. Implemented as a shigoto Job DAG of depth 7. The body of the eighth phase (RECONVERGE) of the universal eight-phase loop in steady state. _Distinct from_ "loop phase" (one of 8) and "Job phase" (one of 10). | CONTINUOUS-SOLUTION-MACHINE.md §IX |
| **promessa** | Tier-1 typed primitive declaring a business-level outcome the cluster must continuously hold (SLA, cost budget, compliance posture, customer KPI, security posture). Authored as `(defpromessa …)`. Reconciled by a PromessaController. Continuously attested via OutcomeChain. The runtime peer of caixa (SDLC), saguão (identity), cofre (secrets) — but expressing _cross-controller business outcomes_, not per-domain declarations. | CONTINUOUS-SOLUTION-MACHINE.md §V |
| **PromessaSpec** | The Rust typed value behind a promessa. `#[derive(TataraDomain, …)]` with `keyword = "defpromessa"`. Lives in `promessa-types` crate. Encodes scope, target, observation source, reconcile interval, remediation policy, escalation ladder, dependencies. | CONTINUOUS-SOLUTION-MACHINE.md §V.3 |
| **PromessaTarget** | The typed outcome a promessa declares. Five canonical kinds: `Sla`, `CostBudget`, `Compliance`, `CustomerKpi`, `Security`. `Custom(…)` for new kinds. Adding a kind = a `TargetController` registration. | CONTINUOUS-SOLUTION-MACHINE.md §V.4 |
| **PromessaController** | The Rust trait that runs the Seven-Beat Convergence Tick on a `PromessaSpec`. Specialization of `Reconciler`. Five trait laws: observation transparency, diff/classify/decide purity, act idempotence on Noop, tick convergence, attest determinism. | CONTINUOUS-SOLUTION-MACHINE.md §V.5 |
| **TargetController** | The per-kind extension point inside the PromessaController registry. Implementations: `SlaController`, `CostBudgetController`, `ComplianceController`, `CustomerKpiController`, `SecurityController`. Each registers via `#[register_target_controller(target_kind = "…")]`. | CONTINUOUS-SOLUTION-MACHINE.md §V.4 |
| **ObservationSource** | Typed binding from a promessa to its data plane. Variants: `Shinryu { query, bindings, materialize }`, `KubeApi`, `HttpEndpoint`, `TameshiHeartbeat`, `CompositeOf(Vec<…>)`. PromessaControllers do not embed raw SQL — they bind a typed source. | CONTINUOUS-SOLUTION-MACHINE.md §V.3 |
| **PromessaDependency** | Typed edge declaring that promessa A depends on promessa B. `DependencyOp` variants: `Meet` (both must hold; act blocks while B is degraded), `Join` (either holds), `Observe` (no blocking; visibility only). Enforced by the per-cluster `PromessaDependencyController`. | CONTINUOUS-SOLUTION-MACHINE.md §X.4 |
| **PromessaLattice** | The runtime peer of the compliance lattice. `meet(a, b)` / `join(a, b)` / `leq(a, b)` over same-kind PromessaSpecs. Cross-kind composition is via PromessaDependency edges, not lattice ops. | CONTINUOUS-SOLUTION-MACHINE.md §X.5 |
| **anomalia** | Tier-1 typed primitive declaring an observable anomaly class (drift, SLO breach, posture regression, budget exceeded, CVE detected). Authored as `(defanomalia …)`. Emitted by any controller (PromessaController, peer-controller, Reconciler). Routed by the AnomalyController. | CONTINUOUS-SOLUTION-MACHINE.md §VI |
| **AnomalySpec** | The Rust typed value behind an anomalia. `#[derive(TataraDomain, …)]` with `keyword = "defanomalia"`. Lives in `anomalia-types` crate. Encodes emitter, kind, severity floor, context schema, remediation policy, escalation ladder reference. | CONTINUOUS-SOLUTION-MACHINE.md §VI.3 |
| **AnomalyController** | Single-replica per-cluster controller subscribed to `anomaly.<cluster>.>` NATS subject. Receives `AnomalyEmission`, resolves `RemediationPolicy`, applies, writes `AnomalyReceipt`. | CONTINUOUS-SOLUTION-MACHINE.md §VI.6 |
| **Severity** | Three-tier classification of drift / anomaly: `Cosmetic` (tolerable, emit receipt only) / `Functional` (user-impacting, default AutoCorrect) / `Critical` (hard violation, default Escalate). Re-exports magma-drift's existing typed enum. | CONTINUOUS-SOLUTION-MACHINE.md §VI.3; CONVERGENCE-SUBSTRATE.md §IV.1 |
| **RemediationPolicy** | Typed response strategy per anomaly: `NoOp` / `Alert` / `AutoCorrect(TypedAction)` / `RequireApproval(group)` / `Escalate(ladder)` / `Compose(Vec<…>)`. Declared with the promessa; applied by the AnomalyController. | CONTINUOUS-SOLUTION-MACHINE.md §VI.4 |
| **TypedAction** | The typed action a RemediationPolicy can take: `FluxCommit { path, patch }`, `ReconcilerApply { kind, config }`, `MagmaApply`, `CofreRotate`, `CrachaPatch`, `Custom`. Per ★★ GITOPS-NATIVE, FluxCommit is preferred. | CONTINUOUS-SOLUTION-MACHINE.md §VI.4 |
| **EscalationLadder** | Ordered sequence of escalation steps with typed time bounds. Steps: `Controller { retry_with }`, `OnCall { who }`, `Manager { who }`, `Exec { who }`, `Custom`. Fires when `RemediationPolicy::Escalate` triggers. Each step's timeout promotes to the next. | CONTINUOUS-SOLUTION-MACHINE.md §VI.5 |
| **EscalationAction** | The typed action a single EscalationStep takes: `Retry`, `Notify`, `OpenIncident`, `QuarantineWorkload`, `BlockChanges`, `Custom`. Composes with `EscalationKind` to define a complete step. | CONTINUOUS-SOLUTION-MACHINE.md §VI.5 |
...
| **OutcomeChain** | Typed, BLAKE3-chained, Ed25519-signed sequence of `OutcomeReceipt`s, one per PromessaController reconcile tick. The runtime peer of tameshi's HeartbeatChain. Proves a promessa was held — or documents exactly when, how, and how badly it was breached — across its lifetime. One chain per promessa per cluster. | CONTINUOUS-SOLUTION-MACHINE.md §VII |
| **OutcomeReceipt** | The typed unit of an OutcomeChain. Carries: promessa_name + namespace, spec_hash (BLAKE3 of canonical PromessaSpec), tick_index, tick_ts, snapshot, snapshot_hash, drift, drift_hash, severity, decision, decision_hash, outcome, outcome_hash, prev_receipt_hash, receipt_hash (BLAKE3 of canonical bytes), signature (Ed25519). Canonical JSON via RFC 8785 for hashing. | CONTINUOUS-SOLUTION-MACHINE.md §VII.2 |
| **AnomalyChain** | Per-cluster typed, BLAKE3-chained, Ed25519-signed sequence of `AnomalyReceipt`s, one per anomaly emission. Append-only. Repeated emissions of the same anomaly dedupe via sliding bloom filter (Critical-severity never dedupes). Records the cluster's full anomaly-handling history. | CONTINUOUS-SOLUTION-MACHINE.md §VIII |
| **AnomalyReceipt** | The typed unit of an AnomalyChain. Carries: cluster, anomaly_id (UUIDv7), tick_ts, emission, resolved_policy, outcome, escalation_path (optional `Vec<EscalationStepResult>`), chain hashes + signature. Records who emitted, what was decided, what was acted on, and how it resolved. | CONTINUOUS-SOLUTION-MACHINE.md §VIII.2 |
| **`kensa verify outcome-chain`** | kensa subcommand that walks an OutcomeChain across a declared window and verifies against a compliance baseline. Returns a typed `OutcomeVerificationReport`. The auditor's deliverable — replaces hand-authored compliance documents. | CONTINUOUS-SOLUTION-MACHINE.md §VII.5; XIV.5 |
| **`kensa replay outcome-chain`** | kensa subcommand that walks an OutcomeChain end-to-end and reconstructs the controller's full trajectory: per-tick spec, observed, drift, severity, decision, outcome. Window aggregates: % of ticks in target, max severity, mean time to remediate. | CONTINUOUS-SOLUTION-MACHINE.md §VII.6 |
| **`kensa replay anomaly-chain`** | kensa subcommand that walks an AnomalyChain across an incident window and renders a typed `PostIncidentReport`. Replaces hand-authored postmortems. | CONTINUOUS-SOLUTION-MACHINE.md §VIII.4 |
| **OutcomeVerificationReport** | The typed Rust value returned by `kensa verify outcome-chain`. Includes: window, baseline, tick counts by severity, remediations taken, chain_root (BLAKE3), signature. The auditor's deliverable as a typed value, not a PDF. | CONTINUOUS-SOLUTION-MACHINE.md §VII.5 |
...
| **promessa** | Promise / commitment. The typed business-outcome primitive. A `(defpromessa …)` value is a typed promise the cluster proves it is continuously holding via the OutcomeChain. Per the ★★ PROVABLE OUTCOMES directive, every operational promise becomes a typed promessa. |
| **anomalia** | Anomaly / deviation. The typed cross-controller anomaly primitive. A `(defanomalia …)` value (or an emitted `AnomalyEmission`) flows through the AnomalyController and is routed per typed RemediationPolicy. The peer of promessa in the dynamic-attestation layer. |
